...
REGULAR_STRING:                      '"'  (~["\\\r\n\u0085\u2028\u2029] | CommonCharacter)* '"';
VERBATIUM_STRING:                    '@"' (~'"' | '""')* '"';
INTERPOLATED_REGULAR_STRING_START:   '$"' { A1(); } -> pushMode(INTERPOLATION_STRING);
INTERPOLATED_VERBATIUM_STRING_START: '$@"' { A2(); }  -> pushMode(INTERPOLATION_STRING);
...
DOUBLE_CURLY_INSIDE:           '{{';
OPEN_BRACE_INSIDE:             '{' { A6(); } -> skip, pushMode(DEFAULT_MODE);
REGULAR_CHAR_INSIDE:           { P1() }? SimpleEscapeSequence;
VERBATIUM_DOUBLE_QUOTE_INSIDE: { P2() }? '""';
DOUBLE_QUOTE_INSIDE:           '"' { A7(); } -> popMode;
REGULAR_STRING_INSIDE:         { P1() }? ~('{' | '\\' | '"')+;
VERBATIUM_INSIDE_STRING:       { P2() }? ~('{' | '"')+;

mode INTERPOLATION_FORMAT;

DOUBLE_CURLY_CLOSE_INSIDE:      '}}' -> type(FORMAT_STRING);
CLOSE_BRACE_INSIDE:             '}' { A8(); }   -> skip, popMode;
FORMAT_STRING:                  ~'}'+;
