...
REGULAR_STRING:                      '"'  (~["\\\r\n\u0085\u2028\u2029] | CommonCharacter)* '"';
VERBATIUM_STRING:                    '@"' (~'"' | '""')* '"';
INTERPOLATED_REGULAR_STRING_START:   '$"' { l.A1(); } -> pushMode(INTERPOLATION_STRING);
INTERPOLATED_VERBATIUM_STRING_START: '$@"' { l.A2(); }  -> pushMode(INTERPOLATION_STRING);
...
DOUBLE_CURLY_INSIDE:           '{{';
OPEN_BRACE_INSIDE:             '{' { l.A6(); } -> skip, pushMode(DEFAULT_MODE);
REGULAR_CHAR_INSIDE:           { p.P1() }? SimpleEscapeSequence;
VERBATIUM_DOUBLE_QUOTE_INSIDE: { p.P2() }? '""';
DOUBLE_QUOTE_INSIDE:           '"' { l.A7(); } -> popMode;
REGULAR_STRING_INSIDE:         { p.P1() }? ~('{' | '\\' | '"')+;
VERBATIUM_INSIDE_STRING:       { p.P2() }? ~('{' | '"')+;

mode INTERPOLATION_FORMAT;

DOUBLE_CURLY_CLOSE_INSIDE:      '}}' -> type(FORMAT_STRING);
CLOSE_BRACE_INSIDE:             '}' { l.A8(); }   -> skip, popMode;
FORMAT_STRING:                  ~'}'+;
